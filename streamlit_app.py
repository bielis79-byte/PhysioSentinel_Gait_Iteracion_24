...
# ------------------------- v0.10.13 · índice global experimental -------------------------
GAIT_INDEX_WEIGHTS = {
    "Ritmo y regularidad": 0.20,
    "Simetría temporal": 0.20,
    "Control frontal de rodilla": 0.20,
    "Control pélvico": 0.15,
    "Tronco y coordinación": 0.10,
    "Pie, retropié y estabilidad": 0.15,
}
...
def _metric_is_usable(metrics, key, allow_experimental=True):
    ...
    blocked = ("no fiable", "no calculable", "suprim", "requiere escala")
    if any(x in q for x in blocked):
        return False
    if (not allow_experimental) and "experimental" in q:
        return False
    return True


def _score_lower_better(value, good, poor):
    """Escala heurística 0-100: 100 <= good; 0 >= poor."""
    try:
        v = float(value)
    except Exception:
        return np.nan
    ...
    if v >= poor:
        return 0.0
    return float(100.0 * (poor - v) / (poor - good))


def _score_higher_better(value, poor, good):
    """Escala heurística 0-100: 0 <= poor; 100 >= good."""
    try:
        v = float(value)
    except Exception:
        return np.nan
    ...
    if v >= good:
        return 100.0
    return float(100.0 * (v - poor) / (good - poor))


def _score_band(value, low, high, tolerance):
    """100 dentro de banda; desciende linealmente hasta 0 fuera de tolerancia."""
    try:
        v = float(value)
    except Exception:
        return np.nan
...
def _index_classification(score):
    if not np.isfinite(score):
        return "No calculable"
    if score >= 85:
        return "Patrón global conservado / alteración mínima"
    if score >= 70:
...
def _domain_classification(score):
    if not np.isfinite(score):
        return "No calculable"
    if score >= 85: return "Conservado"
    if score >= 70: return "Alteración leve"
    if score >= 50: return "Déficit moderado"
    return "Déficit prioritario"


def compute_global_gait_index(metrics):
    """
    Índice compuesto EXPERIMENTAL 0-100 para síntesis clínica y seguimiento.
    No representa '% de normalidad' ni una escala diagnóstica validada.
    Las bandas internas son operativas y transparentes; las métricas ausentes/no
    fiables se excluyen y el peso se redistribuye entre dominios disponibles.
    """
    domains={}
    details={}

    # 1) Ritmo y regularidad. El CV tiene mayor peso. La cadencia se usa solo
    # como referencia contextual, no como objetivo terapéutico rígido.
    rhythm_parts=[]; rhythm_weights=[]; rhythm_detail=[]
    if _metric_is_usable(metrics,"regularity_cv"):
        cv=_metric_float(metrics,"regularity_cv")
        sc=_score_lower_better(cv,5.0,20.0)
        rhythm_parts.append(sc); rhythm_weights.append(0.80)
        rhythm_detail.append(f"CV temporal {cv:.1f}%")
    if _metric_is_usable(metrics,"cadence_exp"):
        cad=_metric_float(metrics,"cadence_exp")
        sc=_score_band(cad,114.95,118.35,20.0)
        rhythm_parts.append(sc); rhythm_weights.append(0.20)
        rhythm_detail.append(f"cadencia {cad:.1f} pasos/min (referencia contextual)")
    domains["Ritmo y regularidad"]=_mean_valid(rhythm_parts,rhythm_weights)
    details["Ritmo y regularidad"]=" · ".join(rhythm_detail) or "Sin métricas suficientes"

    # 2) Simetría temporal. Se prioriza la asimetría global de alternancia.
    sym_parts=[]; sym_weights=[]; sym_detail=[]
    if _metric_is_usable(metrics,"temporal_asymmetry_exp"):
        a=_metric_float(metrics,"temporal_asymmetry_exp")
        sym_parts.append(_score_lower_better(a,5.0,30.0)); sym_weights.append(0.70)
        sym_detail.append(f"asimetría temporal {a:.1f}%")
    if _metric_is_usable(metrics,"stance_asymmetry_2d"):
        a=_metric_float(metrics,"stance_asymmetry_2d")
        sym_parts.append(_score_lower_better(a,5.0,40.0)); sym_weights.append(0.30)
        sym_detail.append(f"asimetría de apoyo {a:.1f}%")
    domains["Simetría temporal"]=_mean_valid(sym_parts,sym_weights)
    details["Simetría temporal"]=" · ".join(sym_detail) or "Sin métricas suficientes"

    # 3) Control frontal de rodilla. Descriptor 2D proyectado; se usa magnitud
    # bilateral para seguimiento, no como diagnóstico de valgo 3D.
    kl=_metric_float(metrics,"dynamic_knee_valgus_l_deg") if _metric_is_usable(metrics,"dynamic_knee_valgus_l_deg") else np.nan
    kr=_metric_float(metrics,"dynamic_knee_valgus_r_deg") if _metric_is_usable(metrics,"dynamic_knee_valgus_r_deg") else np.nan
    kvals=[abs(x) for x in (kl,kr) if np.isfinite(x)]
    if kvals:
        kmax=max(kvals); kmean=float(np.mean(kvals))
        domains["Control frontal de rodilla"]=_mean_valid([
            _score_lower_better(kmax,5.0,50.0),
            _score_lower_better(kmean,5.0,50.0)
        ],[0.60,0.40])
        details["Control frontal de rodilla"]=(
            f"proyección I {kl:.1f}°" if np.isfinite(kl) else "I —"
        ) + (f" · D {kr:.1f}°" if np.isfinite(kr) else " · D —")
    else:
        domains["Control frontal de rodilla"]=np.nan
        details["Control frontal de rodilla"]="Sin métricas suficientes"

    # 4) Control pélvico. Se combinan magnitud máxima y asimetría bilateral.
    pl=_metric_float(metrics,"trendelenburg_drop_l_deg") if _metric_is_usable(metrics,"trendelenburg_drop_l_deg") else np.nan
    pr=_metric_float(metrics,"trendelenburg_drop_r_deg") if _metric_is_usable(metrics,"trendelenburg_drop_r_deg") else np.nan
    pvals=[abs(x) for x in (pl,pr) if np.isfinite(x)]
    if pvals:
        pmax=max(pvals)
        pdiff=abs(pl-pr) if np.isfinite(pl) and np.isfinite(pr) else np.nan
        domains["Control pélvico"]=_mean_valid([
            _score_lower_better(pmax,2.0,12.0),
            _score_lower_better(pdiff,1.0,10.0) if np.isfinite(pdiff) else np.nan,
        ],[0.70,0.30])
        details["Control pélvico"]=(f"drop I {pl:.1f}°" if np.isfinite(pl) else "drop I —") + (f" · D {pr:.1f}°" if np.isfinite(pr) else " · D —")
    else:
        domains["Control pélvico"]=np.nan
        details["Control pélvico"]="Sin métricas suficientes"

    # 5) Tronco y coordinación. Se mantiene como dominio DESCRIPTIVO, pero no
    # se convierte a 0-100 porque r y el desfase no tienen una dirección universal
    # "mejor/peor" transferible a esta medición 2D. Su 10% teórico se redistribuye.
    tdetail=[]
    if _metric_is_usable(metrics,"trunk_pelvis_coupling_r"):
        r=_metric_float(metrics,"trunk_pelvis_coupling_r")
        tdetail.append(f"acoplamiento r={r:.2f}")
    if _metric_is_usable(metrics,"trunk_lateral_lean_rom"):
        rom=abs(_metric_float(metrics,"trunk_lateral_lean_rom"))
        tdetail.append(f"ROM inclinación tronco {rom:.1f}°")
    if _metric_is_usable(metrics,"trunk_pelvis_phase_deg"):
        ph=_metric_float(metrics,"trunk_pelvis_phase_deg")
        tdetail.append(f"desfase {ph:.1f}° de ciclo")
    domains["Tronco y coordinación"]=np.nan
    details["Tronco y coordinación"]=(" · ".join(tdetail) + " · descriptor no puntuado") if tdetail else "Descriptor no puntuado: sin dirección normativa universal"

    # 6) Pie/retropié/estabilidad. Se priorizan diferencias bilaterales; no se
    # juzga la pronación 3D a partir de una sola proyección frontal.
    fparts=[]; fdetail=[]
    if _metric_is_usable(metrics,"foot_progress_diff"):
        v=abs(_metric_float(metrics,"foot_progress_diff"))
        fparts.append(_score_lower_better(v,5.0,30.0)); fdetail.append(f"diferencia orientación pie {v:.1f}°")
    if _metric_is_usable(metrics,"rearfoot_tilt_diff"):
        v=abs(_metric_float(metrics,"rearfoot_tilt_diff"))
        fparts.append(_score_lower_better(v,5.0,25.0)); fdetail.append(f"diferencia retropié {v:.1f}°")
    if _metric_is_usable(metrics,"shoulder_pelvis_rel_rom"):
        v=abs(_metric_float(metrics,"shoulder_pelvis_rel_rom"))
        fparts.append(_score_lower_better(v,8.0,25.0)); fdetail.append(f"ROM hombros-pelvis {v:.1f}°")
    domains["Pie, retropié y estabilidad"]=_mean_valid(fparts)
    details["Pie, retropié y estabilidad"]=" · ".join(fdetail) or "Sin métricas suficientes"

    available={k:float(v) for k,v in domains.items() if np.isfinite(v)}
    weight_available=sum(GAIT_INDEX_WEIGHTS[k] for k in available)
    if not available or weight_available <= 0:
        global_score=np.nan
    else:
        global_score=float(sum(available[k]*GAIT_INDEX_WEIGHTS[k] for k in available)/weight_available)

    coverage=100.0*weight_available/sum(GAIT_INDEX_WEIGHTS.values()) if GAIT_INDEX_WEIGHTS else 0.0
    # Calidad del índice: cobertura + calidad general de tracking/segmento.
    tracking=_metric_float(metrics,"tracking_mean")
    straight=_metric_float(metrics,"straight_walking_usable_pct")
    qparts=[coverage]
    ...
    return {
        ...
        "classification": _index_classification(global_score),
        "domains": domains,
        "details": details,
        ...
        "version": "IGPM-0.1 experimental",
    }


def augment_metrics_with_gait_index(metrics):
    """Añade el índice a la exportación sin duplicar claves."""
    base=[m for m in (metrics or []) if not str(m.get("key","")).startswith("gait_index_")]
    idx=compute_global_gait_index(base)
    rows=[
        {"key":"gait_index_global","label":"Índice Global y Perfil Funcional de la Marcha","value":idx["global_score"],"unit":"/100","quality":idx["confidence_label"],"notes":"Índice compuesto experimental IGPM-0.1. No equivale a porcentaje de normalidad ni a escala diagnóstica validada."},
        {"key":"gait_index_coverage_pct","label":"Cobertura del Índice Global","value":idx["coverage_pct"],"unit":"% peso disponible","quality":idx["confidence_label"],"notes":"Porcentaje de ponderación del índice sustentado por dominios calculables."},
        {"key":"gait_index_confidence","label":"Confianza interna del Índice Global","value":idx["confidence_score"],"unit":"/100","quality":idx["confidence_label"],"notes":"Resumen interno de cobertura, tracking y tramo rectilíneo utilizable; no es validación externa."},
    ]
    key_map={
        "Ritmo y regularidad":"rhythm_regularity",
        "Simetría temporal":"temporal_symmetry",
        "Control frontal de rodilla":"knee_frontal_control",
        "Control pélvico":"pelvic_control",
        "Tronco y coordinación":"trunk_coordination",
        "Pie, retropié y estabilidad":"foot_rearfoot_stability",
    }
    for name,score in idx["domains"].items():
        rows.append({"key":f"gait_index_{key_map[name]}","label":f"Índice de dominio · {name}","value":score,"unit":"/100","quality":_domain_classification(score),"notes":idx["details"].get(name,"")})
    return base+rows
...
tabs = st.tabs([
    ...
    "10 · Índice Global y Perfil Funcional"
])
            ...
            if common_end <= common_start + 1.0:
                ...
                if st.button("Calcular ambas vistas, guardar histórico y eliminar vídeos", type="primary", use_container_width=True):
                    try:
                        ...
                        metrics = augment_metrics_with_gait_index(metrics)
...
with tabs[9]:
    st.subheader("Índice Global y Perfil Funcional de la Marcha")
    st.caption(
        "Síntesis experimental 0–100 para resumir el patrón de marcha, localizar los déficits principales "
        "y facilitar comparaciones longitudinales. No representa un porcentaje de 'normalidad' y no sustituye "
        "la interpretación clínica ni un laboratorio 3D instrumentado."
    )
    ...
    if not metrics_idx:
        ...
        idx = compute_global_gait_index(metrics_idx)
        score = idx["global_score"]
        if not np.isfinite(score):
            st.warning("No hay suficientes dominios válidos para calcular el índice global.")
        else:
            c1,c2,c3,c4 = st.columns([1.15,1.4,1.0,1.0])
            c1.metric("Índice Global", f"{score:.0f}/100")
            c2.metric("Clasificación funcional", idx["classification"])
            c3.metric("Cobertura", f"{idx['coverage_pct']:.0f}%")
            c4.metric("Confianza interna", idx["confidence_label"])
            st.progress(int(np.clip(round(score),0,100)))

            st.info(
                f"**Lectura clínica:** este registro obtiene **{score:.0f}/100** en el índice compuesto "
                f"IGPM-0.1, compatible con **{idx['classification'].lower()}** dentro de esta escala interna. "
                "La puntuación resume los dominios disponibles; no significa que la persona camine ese porcentaje "
                "'bien' o 'normal'."
            )

            st.markdown("### Perfil por dominios")
            rows=[]
            for name,weight in GAIT_INDEX_WEIGHTS.items():
                sc=idx["domains"].get(name,np.nan)
                rows.append({
                    "Dominio":name,
                    "Peso teórico":f"{weight*100:.0f}%",
            ...
            if not plot_df.empty:
                ...
                ax.set_yticks(y)
                ax.set_yticklabels(plot_df["Dominio"].tolist())
                ax.invert_yaxis()
                ax.set_xlim(0,100)
                ax.set_xlabel("Puntuación funcional del dominio (/100)")
                ax.set_title("Perfil funcional de la marcha")
            ...
            st.markdown("### Déficits prioritarios")
            ...
            if ranked:
                ...
                for rank,(name,sc) in enumerate(ranked,1):
                    deficit_rows.append({
                        "Prioridad":rank,
                        "Dominio":name,
                        "Puntuación":round(sc,1),
                        "Déficit relativo":round(100.0-sc,1),
                        "Interpretación":_domain_classification(sc),
                    })
                st.dataframe(pd.DataFrame(deficit_rows),use_container_width=True,hide_index=True)
                worst_name,worst_score=ranked[0]
                best_name,best_score=ranked[-1]
                st.write(
                    f"El dominio con mayor desviación relativa en este registro es **{worst_name}** "
                    f"({worst_score:.0f}/100). El aspecto mejor conservado entre los dominios calculables es "
                    f"**{best_name}** ({best_score:.0f}/100)."
                )

            st.markdown("### ¿Qué tendría que mejorar en una reevaluación?")
            st.write(
                "La evolución favorable no se define únicamente por aumentar el índice global. Debe observarse "
                "mejora en los dominios deficitarios manteniendo o mejorando la calidad del tracking, con el mismo "
                "protocolo de cámara y una velocidad/tarea comparables. Los cambios más útiles son los que se repiten "
                "en varios ciclos y son coherentes con la exploración clínica."
            )

            with st.expander("Metodología del índice y límites",expanded=False):
                st.markdown(
                    """
**IGPM-0.1 experimental.** El índice combina seis dominios: ritmo/regularidad (20%), simetría temporal (20%),
control frontal de rodilla (20%), control pélvico (15%), tronco/coordinación (10%) y pie-retropié/estabilidad (15%).

Las métricas no calculables o marcadas como no fiables no reciben un cero: se excluyen y el peso se redistribuye
entre los dominios disponibles. La cobertura informa qué proporción del índice teórico ha podido utilizarse.

Las transformaciones a 0–100 son reglas operativas internas transparentes para seguimiento longitudinal. El dominio
de tronco/coordinación se muestra de forma descriptiva pero no se puntúa cuando no existe una dirección normativa
universal defendible; su peso se redistribuye entre los dominios calculables. Muchas métricas markerless 2D no
disponen de umbrales diagnósticos universales; por ello este índice **no debe presentarse como una escala validada
de normalidad, riesgo, diagnóstico o pronóstico**. Su utilidad principal es resumir el registro, ordenar los déficits
y comparar cambios cuando se repite el mismo protocolo.
                    """
                )


st.divider()
st.caption("PhysioSentinel Gait v0.10.13 · índice global IGPM-0.1 · perfil funcional por dominios · figura sincronizada única · cursor maestro alineado · exportación")
